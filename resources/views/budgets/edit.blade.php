<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le budget') }} : {{ $budget->name }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('budgets.update', $budget) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <x-text-input id="amount" name="amount" type="number" class="block w-full pl-7" step="0.01" min="0" :value="old('amount', $budget->amount)" required />
                            </div>
                        ...
                        <div>
                            ...
                            <select id="currency_id" name="currency_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                @foreach($currencies as $currency)
                                    <option value="{{ $currency->id }}" @selected(old('currency_id', $budget->currency_id) == $currency->id)>
                        ...
                        <div>
                            ...
                            <select id="period" name="period" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required x-model="period">
                                <option value="monthly" @selected(old('period', $budget->period) == 'monthly')>{{ __('Mensuel') }}</option>
                                <option value="yearly" @selected(old('period', $budget->period) == 'yearly')>{{ __('Annuel') }}</option>
                                <option value="custom" @selected(old('period', $budget->period) == 'custom')>{{ __('Personnalisé') }}</option>
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" :value="old('start_date', $budget->start_date->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('notes', $budget->notes) }}</textarea>
                            <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                        </div>

                        <!-- Actif -->
                        <div class="flex items-center">
                            <input id="active" name="active" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(old('active', $budget->active))>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('budgetForm', () => ({
                period: '{{ old('period', $budget->period) }}'
            }))
        })
    </script>
    @endpush
