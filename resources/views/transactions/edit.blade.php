                    ...
                    <form action="{{ route('transactions.update', $transaction) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required x-model="type">
                                <option value="expense" @selected($transaction->type === 'expense')>{{ __('Dépense') }}</option>
                                <option value="income" @selected($transaction->type === 'income')>{{ __('Revenu') }}</option>
                                <option value="transfer" @selected($transaction->type === 'transfer')>{{ __('Transfert') }}</option>
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="date" type="date" name="date" :value="old('date', $transaction->date->format('Y-m-d'))" required class="mt-1 block w-full" />
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <x-text-input id="amount" type="number" name="amount" :value="old('amount', $transaction->amount)" required class="block w-full pl-7" step="0.01" min="0" />
                            </div>
                        ...
                        <div x-show="type === 'transfer'" x-cloak>
                            ...
                            <select id="destination_account_id" name="destination_account_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" @selected($transaction->transfer_transaction_id && $transaction->transferTransaction->account_id === $account->id)>
                                        {{ $account->name }} ({{ $account->currency->symbol }} {{ number_format($account->current_balance, 2) }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="currency_id" name="currency_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                @foreach($currencies as $currency)
                                    <option value="{{ $currency->id }}" @selected($transaction->currency_id === $currency->id)>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $transaction->description) }}</textarea>
